<?php
// $Id: changemail.php,v 1.4 2007/06/11 09:40:37 mithyt2 Exp $
//  ------------------------------------------------------------------------ //
...
if ($_SERVER['REQUEST_METHOD'] != 'POST' || !isset($_REQUEST['oldmail'])) {
    ...
    if (!isset($_REQUEST['oldmail'])) {
        if (!checkEmail($_POST['newmail'])) {
            ...
            if ($xoopsMailer->send()) {
                ...
                if ($profile_handler->insertUser($xoopsUser)) {
        ...
        //check unique key
        $code = isset($_GET['key']) ? $_GET['key'] : redirect_header(XOOPS_URL, 2, _PROFILE_MA_CONFCODEMISSING);
        if ($code == $key) {
            $profile_handler =& xoops_getmodulehandler('profile');
            $profile = $profile_handler->get($xoopsUser->getVar('uid'));
            //change email address to the proposed one
            $xoopsUser->setVar('email', $profile->getVar('newemail', 'n'));

            // Check user for validity
            include_once(XOOPS_ROOT_PATH."/modules/smartprofile/include/functions.php");
            $stop = userCheck($xoopsUser);
            if (!$stop) {
                //update user data
