<?php
// $Id: field.php,v 1.6 2007/04/11 17:25:37 mithyt2 Exp $
//  ------------------------------------------------------------------------ //
...
switch($op) {
    ...
    unset($categories);
    // AK ADD - START
    // add step information selector details
    $steps[0] = _PROFILE_AM_NOSTEP;
    $criteria = new CriteriaCompo();
    $criteria->setSort('step_order');
    $stepHandler =& xoops_getmodulehandler('regstep');
    $stepObjs =& $stepHandler->getObjects($criteria, true);
    unset($criteria);
    if (count($stepObjs) > 0) {
        foreach (array_keys($stepObjs) as $i) {
            $steps[$stepObjs[$i]->getVar('step_id')] = $stepObjs[$i]->getVar('step_name');
        }
    }
...
//        $fields[$i]['canEdit'] = $fields[$i]['field_config'] || $fields[$i]['field_show'] || $fields[$i]['field_edit']; // AK DELETE
        $fields[$i]['canEdit'] = $fields[$i]['field_config'] || $fields[$i]['field_show'] || $fields[$i]['field_edit'] || $fields[$i]['field_required']; // AK ADD
    ...
    $smartOption['template_main'] = "smartprofile_admin_fieldlist.html";
    break;
...
//            if ($oldweight[$fieldid] != $weight[$fieldid] || $oldcat[$fieldid] != $category[$fieldid]) { // AK DELETE
            if ($oldweight[$fieldid] != $weight[$fieldid] || $oldcat[$fieldid] != $category[$fieldid] || $oldstep[$fieldid] != $step[$fieldid]) { // AK ADD
        ...
        // AK ADD - START
        // set new show_field and req_field settings
        $field_handler =& xoops_getmodulehandler('field');
        foreach ($showFld as $key => $value) {
	        $fields = $field_handler->getObjects(new Criteria('fieldid',$key,"="));
        	$fields[0]->setVar('field_show',intval($value));
        	$fields[0]->setVar('field_required',intval($reqFld[$key]));
        	if (!$field_handler->insert($fields[0])) {
                $errors = array_merge($errors, $fields[0]->getErrors());
            }
            unset($fields);
        }
        unset($field_handler);
        ...
        if (count($ids) > 0) {
            ...
            foreach ($ids as $i) {
                ...
                $fields[$i]->setVar('step_id', intval($step[$i])); //AK ADD
...
// MPB ADD/EDIT - END
    ...
    if ($profilefield_handler->insert($obj)) {
        ...
        $perm_arr = array();
        if ($obj->getVar('field_show')) {
            $perm_arr[] = 'smartprofile_show';
            $perm_arr[] = 'smartprofile_visible';
        }
        if ($obj->getVar('field_edit')) {
            $perm_arr[] = 'smartprofile_edit';
        }
        if ($obj->getVar('field_edit') || $obj->getVar('field_show')) {
            $perm_arr[] = 'smartprofile_search';
        }
        if (count($perm_arr) > 0) {
