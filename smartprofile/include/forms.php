...
function getFieldForm(&$field, $action = false) {
    ...
    if (in_array($field->getVar('field_type'), $searchable_types)) {
        $search_groups = $groupperm_handler->getGroupIds('smartprofile_search', $field->getVar('fieldid'), $GLOBALS['xoopsModule']->getVar('mid'));
        $form->addElement(new XoopsFormSelectGroup(_PROFILE_AM_PROF_SEARCH, 'smartprofile_search', true, $search_groups, 5, true));
    }
    if ($field->getVar('field_edit')) {
        //$form->addElement(new XoopsFormText(_PROFILE_AM_FIELD." "._PROFILE_AM_WEIGHT, 'field_weight', 35, 35, $field->getVar('field_weight', 'e')));
        if (!$field->isNew()) {
            //Load groups
            $editable_groups = $groupperm_handler->getGroupIds('smartprofile_edit', $field->getVar('fieldid'), $GLOBALS['xoopsModule']->getVar('mid'));
...
* @param SmartprofileRegstep $next_step
*
* @return object
*/
function getRegisterForm(&$user, $profile, $next_step = 0, $step) {
    ...
    if ($next_step == 0) {
        ...
        $elements[0][] = array('element' => new XoopsFormText(_PROFILE_MA_USERNAME, "uname", $uname_size, $xoopsModuleConfig['max_uname'], $user->getVar('uname', 'e')), 'required' => true);
        $weights[0][] = 0;
...
// Set field persistance - load profile with session vars
        ...
        if ($value = $_SESSION['smartprofile'][$fieldname]) {
...
// MPB ADD - END
    ...
    foreach (array_keys($elements) as $k) {
        ...
        $reg_form->insertBreak($title, 'head'); // MPB (uncommented)
    ...
    if ($next_step == 0 && $xoopsModuleConfig['display_disclaimer'] != 0 && $xoopsModuleConfig['disclaimer'] != '') {
        ...
        $agree_chk = new XoopsFormCheckBox('', 'agree_disc',$_SESSION['smartprofile']['agree_disc']); // MPB Edit - to keep un/checked
...
function getUserForm(&$user, $profile = false, $action = false) {
    ...
    if ($user->isNew() || $xoopsUser->isAdmin()) {
        $elements[0][] = array('element' => new XoopsFormText(_PROFILE_MA_USERNAME, 'uname', 25, $xoopsModuleConfig['max_uname'], $user->getVar('uname', 'e')), 'required' => 1);
        $email_text = new XoopsFormText('', 'email', 30, 60, $user->getVar('email'));
    } else {
        $elements[0][] = array('element' => new XoopsFormLabel(_PROFILE_MA_USERNAME, $user->getVar('uname')), 'required' => 0); //MPB Label added
    ...
    if ($xoopsUser && $xoopsUser->isAdmin()) {
        if (@!include_once(XOOPS_ROOT_PATH."/modules/smartprofile/language/".$xoopsConfig['language']."/admin.php")) {
            include_once(XOOPS_ROOT_PATH."/modules/smartprofile/language/english/admin.php");
        }
        $gperm_handler =& xoops_gethandler('groupperm');
